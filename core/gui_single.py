...
class SingleFundGUI:
    ...
    支持选择不同基金、实时刷新、数据保存等功能
    """
    
    def __init__(self, root: tk.Tk, funds: list):
        """
        初始化GUI
        ...
        if self.funds:
            self._select_fund(self.funds[0].fund_code)
    
    def _setup_window(self):
        """设置窗口属性"""
        self.root.title("基金实时估值系统 (单基金版)")
    ...
    def _create_fund_selector(self):
        """创建基金选择器"""
        selector_frame = tk.Frame(self.main_container, bg="#2a2a4e")
        ...
        fund_options = [f"{f.fund_code} - {f.fund_name}" for f in self.funds]
        ...
        self.fund_combo.pack(side=tk.LEFT, padx=10, pady=10)
        self.fund_combo.bind("<<ComboboxSelected>>", self._on_fund_selected)
    
    def _create_data_display(self):
        """创建数据显示区域"""
        data_frame = tk.Frame(self.main_container, bg="#2a2a4e")
        data_frame.pack(fill=tk.BOTH, expand=True, pady=10)
        
        inner_frame = tk.Frame(data_frame, bg="#2a2a4e")
    ...
    def _create_control_buttons(self):
        ...
        self.history_btn = tk.Button(
            ...
            relief=tk.FLAT,
            cursor="hand2",
            command=self._show_csv_table,
            width=12,
            height=2
        )
        self.history_btn.pack(side=tk.LEFT, padx=5, pady=5)
    
    def _on_fund_selected(self, event=None):
        """基金选择事件"""
    ...
    def _toggle_auto_refresh(self):
        ...
        if self.auto_refresh_id:
            ...
            self.auto_refresh_id = None
            self.auto_refresh_btn.config(text="▶ 自动刷新", bg="#2ecc71")
            self.status_label.config(text="⏸ 自动刷新已停止")
        else:
            self._start_auto_refresh()
            self.auto_refresh_btn.config(text="⏸ 停止刷新", bg="#e74c3c")
...
def run_single_fund_gui(funds: list):
    """运行单基金GUI"""
    root = tk.Toplevel()
    app = SingleFundGUI(root, funds)
    root.mainloop()
